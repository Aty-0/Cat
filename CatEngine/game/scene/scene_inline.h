...
namespace cat::game::scene
{
	...
	inline T* scene::add(T* g)
	{
		...
		else if (g->get_name().empty() || g->get_id_str().empty())
		{
		...
		VERB("[Scene] [%s] Added [%s] Name: %s UUID: %s at Position (%f %f %f)",
			...
			g->get_name().c_str(), g->get_id_str().c_str(),
		...
		m_storage.push_back(std::move(std::make_pair(std::make_pair(g->get_name(), g->get_id()), go_shared)));
	...
	inline T* scene::clone(std::string object_name, std::string new_name)
	{
		...
		VERB("[Scene] [%s] Clone object Name: %s New name: %s", m_name.c_str(), object_name.c_str(),
		 ...
		 scene_go_storage::iterator it = std::find_if(m_storage.begin(), m_storage.end(),
		 	[&object_name](std::pair<std::pair<std::string, uuids::uuid>, std::shared_ptr<game::game_object>> const& elem)
	...
	template<typename T>
	inline T* scene::clone(uuids::uuid object_id, std::string new_name)
	{
		static_assert(!std::is_base_of<T, game::game_object>::value || std::is_same<T, game::game_object>::value, "This is not GameObject or GameObject based class");

		VERB("[Scene] [%s] Clone object Name: %s New name: %s", m_name.c_str(), core::uuid_object::id_to_str(object_id).c_str(),
			new_name.empty() ? "No" : new_name.c_str());

		 scene_go_storage::iterator it = std::find_if(m_storage.begin(), m_storage.end(),
		 	[&object_id](std::pair<std::pair<std::string, uuids::uuid>, std::shared_ptr<game::game_object>> const& elem)
